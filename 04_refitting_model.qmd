...
Now we can check all the versions of our model that are pinned using the `pin_versions()` function in the **pins** package.  We can also look at Connect and see the new versions of our model - notice the metadata showing the date of the training data used for each version of the model.
```{r}
